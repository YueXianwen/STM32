...
char WiFi_Reset(int timeout)
{
	...
	while(timeout--)                                //等待超时时间到0
	 {                               
		delay_ms(100);                              //延时100ms
		if(strstr(WiFi_RX_BUF,"ready"))  break;     //如果接收到ready表示复位成功，主动跳出while循环
		printf("%d ",timeout);                   //串口输出现在的超时时间
	 }
...
//	printf("准备复位模块\r\n"); 
//	if(WiFi_SendCmd("AT+RST",50)){                //设置STA模式，100ms超时单位，总计5s超时时间
//		printf("复位失败，准备重启\r\n");    //返回非0值，进入if，串口提示数据
//		return 1;                                      //返回2
//	}else printf("复位成功\r\n");            //串口提示数据
   ...
   return 0;                                          //正确返回0，建立起云平台的连接，并进入到了透传的模式
}

...
#include "../Inc/usart3.h"     //包含需要的头文件
#include "../Inc/timer4.h"     //包含需要的头文件
#include "../Inc/mqtt.h"
...
{  	 	
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;                //准备设置PA2   TX
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;                //准备设置PA3   RX
...
void u3_TxData(unsigned char *data)
{
	int	i;	
	while((USART3->SR&0X40)==0);
	for(i = 1;i <= (data[0]*256+data[1]);i++)  //gwd 待发送数据的大小存在待发送数据的前2个字节中，以此来进行数据的发送
	{			
		USART3->DR = data[i+1];                //gwd 一次性发送1个字节
		while((USART3->SR&0X40)==0);	       //gwd 等待发送完成
	} 
}

...
unsigned char *MQTT_TxDataInPtr;                            //指向发送缓冲区存放数据的位置（即将）
unsigned char *MQTT_TxDataOutPtr;                           //指向发送缓冲区读取数据的位置（即将）
...
unsigned char temp_buff[BUFF_UNIT];							//临时缓冲区，构建报文用,512 bytes

char Ping_flag;           //ping报文状态      0：正常状态，等待计时时间到，发送Ping报文   1：Ping报文已发送，当收到服务器回复报文的后 将1置为0												
...
{	
	...
	Ping_flag= 0;                                     //各个参数清零
	Connect_flag = ConnectPack_flag = SubcribePack_flag = 0;  //各个参数清零
}
...
{			
	...
	if(MQTT_TxDataOutPtr != MQTT_TxDataInPtr)         //if成立的话，说明发送缓冲区仍然有数据，未处理  【gwd】
	{
		...
		}else{ //【gwd】在0 1 2 3 4 5 6 在3 4 5 6块存储中时，将OUT指针上移1个单元
		...
		MQTT_TxDataInPtr = MQTT_TxDataOutPtr;          //IN指针指向当前OUT指针  【gwd】:保持输出缓存的2个指向一致！
...
{	
	...
	memset(ServerIP,128,0);  //服务器 ServerIP MQTT连接参数
...
//gwd 进行固定报头的剩余字节长度填充（字节数确定+具体的剩余数据长度）
	do{                                        //循环处理固定报头中的剩余长度字节，字节量根据剩余字节的真实长度变化
		...
		Fixed_len++;	                       //固定报头总长度+1    
	}while(Remaining_len>0);                   //如果Remaining_len>0的话，再次进入循环，固定报头，剩余字节长度是可变长度，根据剩余长度来进行处理，一般是1或2字节
...
//gwd 可变报头部分
	...
	//gwd 协议版本字节
	temp_buff[Fixed_len+6]=0x04;	//可变报头第7个字节 ：固定0x04   //gwd 协议版本
	...
	temp_buff[Fixed_len+9]=0x64;	//可变报头第10个字节：保活时间低字节 0x64   100s   实际服务器端的保活时间是此时间得1.5倍，此情况下是150s
...
{	
	...
	temp_buff[1]=Variable_len + Payload_len;              //第2个字节 ：可变报头+有效负荷的长度	gwd:不会超过128个字节
...
void MQTT_DealPushdata_Qs0(unsigned char *redata)
{
	...
	data = &redata[2];                         //data指向redata的第2个字节，真正的数据开始的，data无符号字符型指针
	...
	for(i=0;i<pack_num;i++){                                        //已经统计到了接收的推送个数，开始for循环，取出每个推送的数据 		
		...
		CMDBuf_Deal(tempbuff, cmd_len);                             //加入命令到缓冲区   gwd:将接收到的来自服务器PUBLISH的数据放置到CMD命令处理缓冲区中，进行后续的处理！
...
void TxDataBuf_Deal(unsigned char *data, int size)
{
	...
	MQTT_TxDataInPtr+=BUFF_UNIT;                 //指针下移，整个下移512字节！！gwd!!! 要弄清楚这里3个指针的具体作用！
	if(MQTT_TxDataInPtr==MQTT_TxDataEndPtr)      //如果指针到缓冲区尾部了
		MQTT_TxDataInPtr = MQTT_TxDataBuf[0];    //指针归位到缓冲区开头，gwd 如果缓存数据过多，下一次会进行覆盖！
    //gwd:上面这个程序的处理是有错误的，最后一块缓冲区实际是没用的
	//gwd:但在这里是没有影响的！
	//gwd:后面是需要注意！可以自己修改进行验证
}
...
{
	memcpy(&MQTT_CMDInPtr[2],data,size);      //拷贝数据到命令缓冲区，从第3个数据开始写！
	MQTT_CMDInPtr[0] = size/256;              //记录数据长度
	MQTT_CMDInPtr[1] = size%256;              //记录数据长度
	MQTT_CMDInPtr[size+2] = '\0';             //加入字符串结束符，字符串结束符的加入！

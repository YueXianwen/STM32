...
static void Write_IIC_Byte(unsigned char IIC_Byte)
{
    ...
    for(i=0;i<8;i++)  //for循环8次
    {
        ...
        IIC_Byte <<= 1;     //数据左移1位
        delay(1);	          //延迟1us
        OLED_SCL=1;         //时钟线置高，产生上升沿，把数据发送出去
        delay(1);	          //延迟1us
    }
    OLED_SCL=0;	          //时钟线置低
    delay(1);	            //延迟1us

...
void W25QXX_Init(void){
    ...
    while (W25QXX_ReadID() != W25Q16){
        ...
        OLED_ShowString(24,2,"W25Q16 ERROR!!");
        delay_ms(200);
        OLED_Clear();
    }
    printf("W25Q16 Check OK!!\r\n");
    Read_Threshold();
}

//初始化SPI FLASH的IO口
void W25QXX_Set(void)
{	
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	...
 	GPIO_SetBits(GPIOB,GPIO_Pin_12); //设置为高电平，PB12为CS片选线，当PB12为低时选中此模块，此模块可进行SPI通信，否则不行
 
    W25QXX_CS=1;	//SPI FLASH 不选中
	SPI2_Init();	//初始化SPI
	SPI2_SetSpeed(SPI_BaudRatePrescaler_2);//设置为18M时钟,高速模式；2分频，36Mhz/2，即18Mhz
...
{ 
	...
	while(1) 
	{	
		W25QXX_Read(W25QXX_BUF,secpos*256,256);//读出整个扇区的内容
		...
		if(i<secremain)//需要擦除
		{
			...
			W25QXX_Write_NoCheck(W25QXX_BUF,secpos*256,256);//写入整个扇区
		...
		else//写入未结束
		{
			...
			if(NumByteToWrite>256)secremain=256;//下一个扇区还是写不完

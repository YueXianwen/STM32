				    ...
				    switch(MQTT_RxDataOutPtr[5]) //接着我们要判断第4个字节，看CONNECT报文是否成功，根据发回的信息判断错误形式
						{					
							case 0x00 : printf("CONNECT报文成功\r\n");                          //串口输出信息	
										...
										TIM3_ENABLE_30S();                  	//【gwd】启动30s的PING定时器
										break;                                                    	//跳出分支case 0x00                                              
				...
				{ 
					...
					if(Ping_flag==1)
						...
						{ 				               //如果Ping_flag>1，表示是多次发送了，而且是2s间隔的快速发送
							Ping_flag = 0;     		 //要清除Ping_flag标志
							TIM3_ENABLE_30S(); 		 //PING定时器重回30s的时间,30s时间会Ping一次，以此来确定保活
						}				
				}	
...
//
//				if(strstr((char *)MQTT_CMDOutPtr+2,"\"params\":{\"LightSwitch\":1}")){	       //如果搜索到"params":{"PowerSwitch":1}说明服务器下发打开开关1
//					LED0=0;                                                                    //打开LED1
//					LED0_State();                                                              //判断开关状态，并发布给服务器
//				}else if(strstr((char *)MQTT_CMDOutPtr+2,"\"params\":{\"LightSwitch\":0}")){   //如果搜索到"params":{"PowerSwitch":0}说明服务器下发关闭开关1
//					LED0=1;                                                                    //关闭LED1
//					LED0_State();                                                              //判断开关状态，并发布给服务器
//				}
//
...
//			}
			...
			{   			         
				...
				TIM3_ENABLE_5S();                  //【gwd】启动5sPING检测处理【gwd】前置Ping的报文处理，避免CONNECT的处理问题
			}				
		}
}
...
{
    char temp[BUFF_UNIT];                   //定义一个临时缓冲区
    if(BEEP_IN_STA) sprintf(temp,"{\"method\":\"thing.event.property.post\",\"id\":\"203302322\",\"params\":{\"BEEP\":0},\"version\":\"1.0.0\"}");  //如果LED1是高电平，说明是熄灭状态，需要回复关闭状态给服务器
    else  sprintf(temp,"{\"method\":\"thing.event.property.post\",\"id\":\"203302322\",\"params\":{\"BEEP\":1},\"version\":\"1.0.0\"}");  //如果LED1是低电平，说明是点亮状态，需要回复打开状态给服务器
    MQTT_PublishQs0(P_TOPIC_NAME,temp,strlen(temp));   //添加数据，发布给服务器
}
